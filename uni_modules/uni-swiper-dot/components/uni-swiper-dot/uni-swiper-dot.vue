<template>
	<view class="uni-swiper__warp">
		...
		<view v-if="mode === 'default'" :style="{'bottom':dots.bottom + 'px'}" class="uni-swiper__dots-box"
			key='default'>
			<view v-for="(item,index) in info" @click="clickItem(index)"
				:style="{
		...
		<view v-if="mode === 'nav'" key='nav' :style="{'background-color':dotsStyles.backgroundColor,'bottom':'0'}"
			...
			<text :style="{'color':dotsStyles.color}" 
				class="uni-swiper__dots-nav-item">{{info.length == 0? '': info[current][field]}}</text>
			<text :style="{'color':dotsStyles.color}" style="position: absolute; right: 0;"
				class="uni-swiper__dots-nav-item">{{info.length == 0? '':  info[current][val] }}</text>
		</view>
		<view v-if="mode === 'indexes'" key='indexes' :style="{'bottom':dots.bottom + 'px'}"
        ...
        'width':dots.width + 'px','height':dots.height +'px' ,'color':index === current?dots.selectedColor:dots.color,'background-color':index !== current?dots.backgroundColor:dots.selectedBackgroundColor,'border':index !==current ? dots.border:dots.selectedBorder}"
				:key="index" class="uni-swiper__dots-item uni-swiper__dots-indexes"><text
					class="uni-swiper__dots-indexes-text">{{ index+1 }}</text></view>
		</view>
	</view>
</template>
