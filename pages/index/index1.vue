<template>
	<view>
		...
		<view class="page" @touchstart="start" @touchend="end" @touchmove="move">
			...
			<!-- 导航栏 -->
			<view class="content" @click="navItemClick(2, showInfo.path)">
				<span class="icontext">公告</span>
				<span style="display: inline;overflow: hidden;position: absolute;left: 105rpx;">
					<span class="text" :style="{ marginLeft: marginLeft }">
						{{ showInfo.informationTitle + '&#12288;&#12288;&#12288;&#12288;&#12288;' + showInfo.informationTitle }}
					</span>
				</span>
			</view>
			<!-- 			<view class="box onliyTop2">
				<view class="nav onliyTop">
	...
	export default {
		data() {
			return {
				...
				showInfo: {
					informationTitle: '杭州市滨江区封控区、管控区和部分防范区范围内（钱塘山水小区、知本大厦、杭州高新软件园9幢、源越大厦）。',
					path: ''
				},
				inTime: 0,
				margint: 200,
		...
		mounted() {
			let that = this;
			this.inTime = setInterval(that.setInfo, 10);
		},
		beforeDestroy() {
		...
		methods: {
			...
			setInfo() {
				/* 			this.showInfo = this.showInfo+this.showInfo.charAt(0);
				this.showInfo = this.showInfo.substring(1,this.showInfo.length); */
				this.margint--;
				if (this.margint < -(this.showInfo.informationTitle.length * 30 + 150)) {
					this.margint = 0;
				}
			},
			start(e) {
			...
			getLastInformation() {
				this.$request({
						path: '/api/activity/selectLastInfo',
						method: 'GET',
						params: {}
					})
					.then(value => {
						this.showInfo = value.data;
						this.showInfo.path = `/pages/news/components/newDetails?id=${value.data.id}`;
					})
					.catch(reason => {
						console.error(reason);
					});
