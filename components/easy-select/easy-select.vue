<template>
	<view class="easy-select" @click.stop="trigger" :style="[easySelectSize]">
		...
		color: #888;" type="text" v-model="label" :placeholder="placeholder" disabled clearable>
		<view class="easy-select-suffix" :style="{border: '0px solid rgba（0,0,0,0)'}" :class="[showSuffix]">
			<view style="font-size: 28rpx;" class="easy-select-down-tag">▲</view>
		</view>
		<view class="easy-select-options" v-if="showOptions" :style="{'min-width': boundingClientRect.width + 'px', top: '30rpx', margin: optionsGroupMargin}">
